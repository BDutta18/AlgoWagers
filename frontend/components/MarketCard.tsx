...
export function MarketCard({
  market, active, onClick
}: {
  market: Market, active: boolean, onClick: () => void
}) {
  ...
  const totalPool = market.yesPoolALGO + market.noPoolALGO
  const yesProb = Math.round((market.yesPoolALGO / totalPool) * 100)
      ...
      >
        ...
        >
          <div className="flex flex-col">
            <span style={{ fontFamily: 'var(--font-logo)', fontSize: '0.65rem', color: 'rgba(255,255,255,0.7)', letterSpacing: '0.05em' }}>MARKET_TICKER</span>
            <span style={{ fontFamily: 'var(--font-logo)', fontSize: '0.9rem', color: '#FFF', letterSpacing: '0.15em' }}>{market.ticker}</span>
          </div>
        ...
        <motion.div className="relative z-[2]" style={{ transform: "translateZ(50px)" }}>
           <div style={{ 
            ...
            {yesProb}<span style={{fontSize: '2rem'}}>% YES</span>
          </div>
        ...
        <motion.div className="mt-auto relative z-[2] pt-8" style={{ transform: "translateZ(20px)" }}>
          ...
          >
            VOL: {market.volume.toLocaleString()} ALGO | CLOSE: {market.closeTime}
          </div>
          <div className="font-[family-name:--font-body] text-[0.95rem] font-bold text-white uppercase tracking-[0.02em] leading-tight mt-1">
            {market.question}
          </div>
        </motion.div>
